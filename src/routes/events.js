...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE events SET name = $1, date = $2, location = $3, description = $4
       WHERE id = $5 AND user_id = $6 RETURNING *`,
      [name, date, location, description, id, req.user.id]
    );
    ...
    res.json({ message: 'Evento actualizado', evento: result.rows[0] });
